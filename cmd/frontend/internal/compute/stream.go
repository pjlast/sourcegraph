...
func (h *streamHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	matchesBuf := streamhttp.NewJSONArrayBuf(32*1024, func(data []byte) error {
		return eventWriter.EventBytes("results", data) // TODO need better descriptor here? or in compute/stream.go to capture repos
	})
	matchesFlush := func() {
	...
	first := true
	handleEvent := func(event compute.Event) {
...
func parseURLQuery(q url.Values) (*args, error) {
	...
	display := get("display", "-1") // FIXME TODO unused currently, no limit for compute result.
	var err error
...
func batchEvents(source <-chan compute.Event, delay time.Duration) <-chan compute.Event {
	results := make(chan compute.Event)
	go func() {
		defer close(results)
