...
func (c metricsCollector) Collect(ch chan<- prometheus.Metric) {
	metric := func(desc *prometheus.Desc, value float64) prometheus.Metric {
		return prometheus.MustNewConstMetric(desc, prometheus.GaugeValue, value)
	}

	stats := c.db.Stats()
	ch <- metric(c.maxOpenDesc, float64(stats.MaxOpenConnections))
	ch <- metric(c.openDesc, float64(stats.OpenConnections))
	ch <- metric(c.inUseDesc, float64(stats.InUse))
	ch <- metric(c.idleDesc, float64(stats.Idle))
	ch <- metric(c.waitedForDesc, float64(stats.WaitCount))
	ch <- metric(c.blockedSecondsDesc, stats.WaitDuration.Seconds())
	ch <- metric(c.closedMaxIdleDesc, float64(stats.MaxIdleClosed))
	ch <- metric(c.closedMaxLifetimeDesc, float64(stats.MaxLifetimeClosed))
	ch <- metric(c.closedMaxIdleTimeDesc, float64(stats.MaxIdleTimeClosed))
}
